        ...
        public override void ApplyForce(float dt, float strength)
        {
            foreach (Body body in World.BodyList)
            {
                ...
                if (decayMultiplier != 0)
                {
                    ...
                    //forceVector = Vector2.Transform(forceVector, Matrix.CreateRotationZ((MathHelper.Pi -Microsoft.Xna.Framework.MathHelper.Pi/2) * (float)Randomize.NextDouble()));

                    // Calculate random Variation
                    if (Variation != 0)
                    {
                        float strengthVariation = (float)Randomize.NextDouble() *Microsoft.Xna.Framework.MathHelper.Clamp(Variation, 0, 1);

...
public class Demo : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        if (showGlowControls) {
            ...
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("<-")) {
                blendMode = (GlowEffect.BlendMode)(((int)blendMode - 1) % 3);
                if ((int)blendMode < 0) blendMode = GlowEffect.BlendMode.Screen;
                updateGlow();
            }
            if (GUILayout.Button("->")) {
                blendMode = (GlowEffect.BlendMode)(((int)blendMode + 1) % 3);
    ...
    private void updateGlow()
    {
        ...
        if (enableGlow) {
#if UNITY_3_5
            glowGroup.SetActiveRecursively((int)glowMode % 2 == 0);
            alphaGlowGroup.SetActiveRecursively((int)glowMode % 2 == 1);
#else

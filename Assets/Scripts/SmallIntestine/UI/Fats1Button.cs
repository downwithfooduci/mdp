...
public class Fats1Button : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		// the fats1 button is the first button from the left
		buttonLeft = Screen.width * 0.0148f;					// set the button left coordinate relative to screen size
	...
	void Update () 
	{
		...
		{	
			foreach (Touch touch in Input.touches) 
			{
				...
				{
					GetComponent<RawImage>().texture = pressedTexture;		// if the button is being pressed draw the pressed texture
				...
				{
					// when we aren't pressing a button then draw the "active" texture

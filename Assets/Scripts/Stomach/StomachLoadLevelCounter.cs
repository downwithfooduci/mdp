...
public class StomachLoadLevelCounter : MonoBehaviour 
{
	...
	void Start () 
	{
		level = PlayerPrefs.GetInt ("DesiredStomachLevel");		// pull the desired mouth level from playerprefs
		
		if (level != -1)										// check if the level is -1
			// -1 means that we are starting the game fresh
		{
			// reset the variable for later use
			PlayerPrefs.SetInt("DesiredStomachLevel", -1);		// after we read rewrite the default value to stats
			PlayerPrefs.Save ();
		} else
		{
		...
		if (GameObject.FindGameObjectsWithTag ("stomachBackgroundChooser").Length > 1)
		{
			Destroy (gameObject);
		}
	}
	...
	void Update () 
	{
		...
		if (scene != "LoadLevelStomach" && scene != "Stomach" && scene != "StomachStats")
		{
			Destroy (gameObject);
		}
	}

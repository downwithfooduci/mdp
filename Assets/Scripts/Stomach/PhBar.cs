...
public class PhBar : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if (startAddAcid)
		{
			...
			moveCurrentLevelRect(startingAcidSpeed * Time.deltaTime * (768f / Screen.height) * 
			                     ( 1 - (elapsedTime / acidSpeedDecayTime)));
		...
		if (startAddBase)
		{
			...
			// it should move at a decreasing speed so to do so we multiply the desired speed by the percentage of time
			// that is left to move the indicator line
			moveCurrentLevelRect(startingBaseSpeed * Time.deltaTime * (768f / Screen.height) * 
			                     ( 1 - (elapsedTime / baseSpeedDecayTime)));

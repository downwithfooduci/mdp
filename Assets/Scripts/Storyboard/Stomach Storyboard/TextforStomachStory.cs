...
public class TextforStomachStory : MonoBehaviour
{

    // Use this for initialization
    StoryboardHandler StomachStoryboard;
    private string[] text;
    private float timer;
    ...
    void Start()
    {
        StomachStoryboard = this.gameObject.GetComponent<StoryboardHandler>();

        TextAsset introText = Resources.Load("1.13.2016NewText/StomachStory") as TextAsset;
    ...
    void OnGUI()
    {
        ...
        if (StomachStoryboard.getCurrPage() == 1)
        {
        ...
        if (StomachStoryboard.getCurrPage() == 2)
        {
            GUI.Box(new Rect(0.001f * Screen.width, (665f / 768f) * Screen.height, 1.0f * Screen.width,
                               .2f * Screen.height), text[1], statsStyle);
            timer = 0;
        }
        if (StomachStoryboard.getCurrPage() == 3)
        {
            GUI.Box(new Rect(0.001f * Screen.width, (665f / 768f) * Screen.height, 1.0f * Screen.width,
                               .2f * Screen.height), text[2], statsStyle);
            timer = 0;
        }
        if (StomachStoryboard.getCurrPage() == 4)
        {
            GUI.Box(new Rect(0.001f * Screen.width, (665f / 768f) * Screen.height, 1.0f * Screen.width,
